    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
      <div style={{ background: "#111118", border: "1px solid #1e1e2e", borderRadius: 20, padding: 36, width: 380, boxShadow: "0 30px 60px rgba(0,0,0,.5)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
          ...
          <button onClick={onClose} style={{ background: "transparent", color: "#555", fontSize: 20 }}>×</button>
        </div>
        {ok ? (
          ...
          <>
            {[
              ...
              <div key={f.label}>
                ...
                <input type="password" value={f.val} onChange={e => { f.set(e.target.value); setErr(""); }} style={inp}
...
function ClientView({ user, onLogout }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data: coch } = await supabase.from("cocheras").select("*").eq("cliente_id", user.id).eq("activo", true);
      const { data: pags } = await supabase.from("pagos").select("*").eq("cliente_id", user.id).order("anio").order("mes");
      setCocheras(coch || []);
      setPagos(pags || []);
      setLoading(false);
    };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", fontFamily: "'DM Sans', sans-serif", color: "#e5e5e5" }}>
      ...
      {showCambioPass && (
        <ModalCambioPassword user={currentUser} onClose={() => {
      ...
      <div style={{ background: "#111118", borderBottom: "1px solid #1e1e2e", padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 12, color: "#666" }}>{cocheras.length} lugar{cocheras.length !== 1 ? "es" : ""} alquilado{cocheras.length !== 1 ? "s" : ""}</div>
          </div>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 20px" }}>
        <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: 24, fontWeight: 800, color: "#fff", marginBottom: 20 }}>Mi cuenta</h2>

        {/* Cocheras del cliente */}
        {cocheras.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ color: "#666", fontSize: 12, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 }}>Mis lugares</div>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {cocheras.map(c => (
                <div key={c.id} style={{ background: "#111118", border: "1px solid #1e1e2e", borderRadius: 12, padding: "12px 18px", display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 22 }}>{TIPO_ICON[c.tipo] || "🅿️"}</span>
                  <div>
                    <div style={{ fontWeight: 700, color: "#fff", fontSize: 14 }}>{c.nombre}</div>
                    <div style={{ fontSize: 12, color: "#666", textTransform: "capitalize" }}>{c.tipo} · {fmt(c.monto_mensual)}/mes</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 24 }}>
          {[
            ...
            { label: "Cuota mensual total", val: fmt(totalMensual), color: "#7c3aed" },
          ].map(st => (
...
function AdminView({ onLogout }) {
  ...
  const selectCliente = (c) => { setSelected(c); fetchDetalle(c.id); setShowAddPago(false); setShowAddCochera(false); };
  ...
  const deleteCochera = async (cocheraId, nombre) => {
    if (!window.confirm(`¿Eliminar el lugar "${nombre}"?`)) return;
    await supabase.from("cocheras").update({ activo: false }).eq("id", cocheraId);
    fetchDetalle(selected.id);
  };

  const inp = { background: "#080b10", border: "1px solid #1a2030", borderRadius: 8, padding: "9px 14px", color: "#fff", fontSize: 14, outline: "none" };
  const totalMensual = cocheras.reduce((a,c) => a + Number(c.monto_mensual), 0);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080b10", fontFamily: "'DM Sans', sans-serif", color: "#e5e5e5" }}>
      ...
      <div style={{ width: 270, background: "#0d1117", borderRight: "1px solid #1a2030", position: "fixed", top: 65, left: 0, bottom: 0, overflowY: "auto", padding: "20px 12px" }}>
        ...
        : clientes.map(c => (
          <div key={c.id} style={{ padding: "12px 14px", borderRadius: 10, marginBottom: 5, background: selected?.id===c.id ? "#131e35" : "transparent", border: selected?.id===c.id ? "1px solid #1e3a5f" : "1px solid transparent" }}>
            <div onClick={() => selectCliente(c)} style={{ cursor: "pointer" }}>
              <div style={{ fontWeight: 600, color: "#fff", fontSize: 14 }}>{c.nombre}</div>
              <div style={{ fontSize: 12, color: "#555", marginTop: 2 }}>{c.email}</div>
            </div>
            <button onClick={async (e) => {
              e.stopPropagation();
              if (!window.confirm(`¿Eliminar a ${c.nombre}? Se borrarán todos sus datos.`)) return;
        ...
        {!selected ? (
          <>
            ...
            <div style={{ background: "#0d1117", border: "1px solid #1a2030", borderRadius: 16, padding: 24 }}>
              <div style={{ fontWeight: 700, color: "#fff", marginBottom: 16 }}>Seleccioná un cliente para ver sus lugares y pagos</div>
              {clientes.map((c, i) => (
                <div key={c.id} onClick={() => selectCliente(c)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: i<clientes.length-1 ? "1px solid #151d2a" : "none", cursor: "pointer" }}>
                  ...
                  <span style={{ fontSize: 13, color: "#3b82f6" }}>Ver →</span>
                </div>
              ))}
            </div>
          </>
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 24 }}>
              {[
                ...
                { label: "Cuota mensual total", val: fmt(totalMensual), color: "#a78bfa" },
              ].map(st => (
            ...
            {/* Cocheras del cliente */}
            <div style={{ background: "#0d1117", border: "1px solid #1a2030", borderRadius: 16, padding: 24, marginBottom: 16 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <div style={{ fontWeight: 700, color: "#fff" }}>Lugares alquilados</div>
                <button onClick={() => setShowAddCochera(!showAddCochera)} style={{ background: "#1e3a5f", color: "#60a5fa", padding: "7px 14px", borderRadius: 8, fontSize: 13, fontWeight: 600 }}>
                  {showAddCochera ? "Cancelar" : "+ Agregar lugar"}
                </button>
              </div>

              {showAddCochera && (
                <div style={{ background: "#080b10", border: "1px solid #1a2030", borderRadius: 12, padding: 14, marginBottom: 14, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                  <div>
                    <div style={{ color: "#555", fontSize: 12, marginBottom: 4 }}>Nombre / Número</div>
                    <input placeholder="Ej: A-03" value={newCochera.nombre} onChange={e => setNewCochera({...newCochera, nombre: e.target.value})}
                      style={{ ...inp, width: 110 }} />
                  </div>
                  <div>
                    <div style={{ color: "#555", fontSize: 12, marginBottom: 4 }}>Tipo</div>
                    <select value={newCochera.tipo} onChange={e => setNewCochera({...newCochera, tipo: e.target.value})}
                      style={{ ...inp, width: 120, background: "#0d1117" }}>
                      {TIPOS.map(t => <option key={t} style={{ background: "#0d1117" }}>{t}</option>)}
                    </select>
                  </div>
                  <div>
                    <div style={{ color: "#555", fontSize: 12, marginBottom: 4 }}>Monto mensual $</div>
                    <input type="number" placeholder="15000" value={newCochera.monto_mensual} onChange={e => setNewCochera({...newCochera, monto_mensual: e.target.value})}
                      style={{ ...inp, width: 130 }} />
                  </div>
                  <button onClick={addCochera} style={{ background: "linear-gradient(135deg,#7c3aed,#4f46e5)", color: "#fff", padding: "9px 18px", borderRadius: 8, fontWeight: 700, fontSize: 13 }}>Guardar</button>
                </div>
              )}

              {loadingDetalle ? <div style={{ color: "#555", fontSize: 13 }}>Cargando...</div>
              : cocheras.length === 0 ? <div style={{ color: "#555", fontSize: 13 }}>Sin lugares asignados. Agregá uno con el botón de arriba.</div>
              : (
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  {cocheras.map(c => (
                    <div key={c.id} style={{ background: "#080b10", border: "1px solid #1a2030", borderRadius: 12, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontSize: 22 }}>{TIPO_ICON[c.tipo] || "🅿️"}</span>
                      <div>
                        <div style={{ fontWeight: 700, color: "#fff", fontSize: 14 }}>{c.nombre}</div>
                        <div style={{ fontSize: 12, color: "#555", textTransform: "capitalize" }}>{c.tipo} · {fmt(c.monto_mensual)}/mes</div>
                      </div>
                      <button onClick={() => deleteCochera(c.id, c.nombre)}
                        style={{ background: "transparent", color: "#ef4444", fontSize: 16, padding: "0 0 0 8px", border: "none", cursor: "pointer" }}>×</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div style={{ background: "#0d1117", border: "1px solid #1a2030", borderRadius: 16, padding: 24 }}>
                ...
                <div style={{ background: "#080b10", border: "1px solid #1a2030", borderRadius: 12, padding: 16, marginBottom: 16, display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
                  <div>
                    ...
                    <select value={newPago.mes} onChange={e => setNewPago({...newPago, mes: e.target.value})}
                      style={{ ...inp, width: 130, background: "#0d1117" }}>
                  ...
                  <div>
                    ...
                    <input type="number" value={newPago.anio} onChange={e => setNewPago({...newPago, anio: e.target.value})}
                      style={{ ...inp, width: 90, background: "#0d1117" }} />
                  </div>
                  <div>
                    <div style={{ color: "#555", fontSize: 12, marginBottom: 4 }}>Monto $</div>
                    <input type="number" value={newPago.monto} placeholder={fmt(totalMensual)}
              ...
              {loadingDetalle ? <div style={{ textAlign: "center", color: "#555", padding: 30 }}>Cargando...</div>
